...
assert.equal(steps.dismissArchiveSurface.primitive, "keyboard.press");
assert.equal(steps.dismissArchiveSurface.args.key, "Escape");
assert.equal(steps.closeCard.primitive, "keyboard.press");
assert.equal(steps.closeCard.args.key, "Escape");
assert.equal(steps.verifyCardClosed.primitive, "dom.observe.visible");
assert.equal(steps.verifyCardClosed.args.selector, "[data-testid='card-back-name']");
assert.match(steps.verifyCardClosed.retry_until, /match_count = 0/);
assert.equal(steps.verifyCardClosed.on_error, "continue");
assert.equal(steps.verifyCardClosed.after_each, undefined);
